...
from simulate import (simulate_free, simulate_lqr, simulate_lqg,
                      ...
                      plot_bode, plot_pole_zero, plot_phase_portrait,
                      plot_summary_dashboard, animate_comparison,
...
def main():
    ...
    # Build metrics dict for downstream functions
    freq_error_mode1 = abs(
        100.0 * (freqs_fem[0] - freqs_analytical[0]) / freqs_analytical[0]
    )
    metrics = {
        'settling_free':     settling_free,
        'settling_lqr':      settling_lqr,
        'settling_lqg':      settling_lqg,
        'peak_lqr':          float(np.max(np.abs(u_lqr))),
        'peak_lqg':          float(np.max(np.abs(u_lqg))),
        'rms_lqr':           float(np.sqrt(np.mean(u_lqr**2))),
        'rms_lqg':           float(np.sqrt(np.mean(u_lqg**2))),
        'n_states':          A.shape[0],
        'n_dof_free':        fem.n_dof_free,
        'freq_error_mode1':  freq_error_mode1,
    }

    # =========================================================================
    # 14. Bode magnitude plot
    # =========================================================================
    print("\n" + "="*60)
    print("STEP 14: BODE MAGNITUDE PLOT")
    print("="*60)

    plot_bode(A, B, C, lqr.K, filename='bode_plot.png')

    # =========================================================================
    # 15. Pole-zero map
    # =========================================================================
    print("\n" + "="*60)
    print("STEP 15: POLE-ZERO MAP")
    print("="*60)

    plot_pole_zero(A, B, lqr.K, filename='pole_zero.png')

    # =========================================================================
    # 16. Phase portrait
    # =========================================================================
    print("\n" + "="*60)
    print("STEP 16: PHASE PORTRAIT")
    print("="*60)

    plot_phase_portrait(
        t_free, x_free, t_lqr, x_lqr, t_lqg, x_lqg,
        sensor_dof=ss.sensor_dof,
        n_dof_free=fem.n_dof_free,
    ...
    # =========================================================================
    print("\n" + "="*60)
    print("STEP 17: SUMMARY DASHBOARD")
    print("="*60)
    ...
    # =========================================================================
    # 18. Side-by-side comparison animation
    # =========================================================================
    print("\n" + "="*60)
    print("STEP 18: COMPARISON ANIMATION")
    print("="*60)

    print("  Creating side-by-side free vs LQG animation...")
    animate_comparison(
        fem,
        t_free, x_free, y_free,
        t_lqg,  x_lqg,  y_lqg,  u_lqg,
        filename='beam_comparison.gif',
        fps=30, duration=2.0
    )
    ...
    # =========================================================================
    print("\n" + "="*60)
    print("STEP 19: HTML REPORT")
    print("="*60)

    generate_html_report(
        figure_files={
            'Summary Dashboard':         'summary_dashboard.png',
            'Mode Shapes (colormap)':    'mode_shapes.png',
            'Bode Magnitude Plot':       'bode_plot.png',
            'Pole-Zero Map':             'pole_zero.png',
            'Transient Response (+ FFT)':'transient_response.png',
            'Phase Portrait':            'phase_portrait.png',
            'Free Vibration Animation':  'beam_free.gif',
            'LQR Control Animation':     'beam_lqr.gif',
            'LQG Control Animation':     'beam_lqg.gif',
            'Comparison Animation':      'beam_comparison.gif',
        },
        metrics=metrics,
    ...
    print("    - mode_shapes.png          (Modal analysis — colormap)")
    print("    - bode_plot.png            (Bode magnitude — OL vs CL)")
    print("    - pole_zero.png            (Pole-zero map)")
    print("    - transient_response.png   (Transient + FFT)")
    print("    - phase_portrait.png       (Phase portrait)")
    print("    - summary_dashboard.png    (Portfolio dashboard)")
    print("  Animations:")
    print("    - beam_free.gif            (Free vibration)")
    print("    - beam_lqr.gif             (LQR control — with inset + force)")
    print("    - beam_lqg.gif             (LQG control — with inset + force)")
    print("    - beam_comparison.gif      (Side-by-side Free vs LQG)")
    print("  Report:")
    print("    - report.html              (Self-contained HTML report)")
    print("\nPerformance Summary:")
    print(f"  Vibration suppression: {(1 - settling_lqr/settling_free)*100:.1f}% faster settling")
